             ...
             const erlcm_rect_list_t *msg, void *user)
{
    ...
    if (!strcmp (channel, "MAP_SERVER_RECTS"))
        message_buffer_give(self->map_rects_buffer, erlcm_rect_list_t_copy(msg));
    else
        message_buffer_give(self->sim_rects_buffer, erlcm_rect_list_t_copy(msg));
}
...
{
    check_gridmap_t *self = (check_gridmap_t*) user;
    message_buffer_give(self->rects_buffer, erlcm_rect_list_t_copy(&msg->rects));
    //message_buffer_give(self->tracks_buffer, erlcm_track_list_t_copy(&msg->tracks));
...
static void draw_obs_gridmap(check_gridmap_t *self, gridmap_t *gm)
{
    ...
    //-------------------------
    //--- Static rects
    erlcm_rect_list_t *rects = message_buffer_get(self->rects_buffer);
    ...
    */
    ...
    //--- tracks
    if(!self->clear_person){
        erlcm_track_list_t *tracks = message_buffer_get(self->tracks_buffer);
        for (int i = 0; tracks && i < tracks->ntracks; i++) {
            
            //not sure what will happen here 
            
            erlcm_track_t *track = &tracks->tracks[i];
            // render actual obstacle
        ...
        //-------------------------
        //--- sim tracks
        erlcm_track_list_t *simtracks = message_buffer_get(self->sim_tracks_buffer);
        for (int i = 0; simtracks && i < simtracks->ntracks; i++) {
            erlcm_track_t *track = &simtracks->tracks[i];
            // render actual obstacle
            gridmap_render_rectangle_restricted(gm, track->pos[0], track->pos[1],
                                                track->size[0], track->size[1],
                                                track->theta,
                                      ...
                                      double width_buffer)
{
    ...
    self->rects_buffer = message_buffer_create((message_buffer_free_func_t) erlcm_rect_list_t_destroy);
    self->sim_rects_buffer = message_buffer_create((message_buffer_free_func_t) erlcm_rect_list_t_destroy);
    self->map_rects_buffer = message_buffer_create((message_buffer_free_func_t) erlcm_rect_list_t_destroy);
    self->tracks_buffer = message_buffer_create((message_buffer_free_func_t) erlcm_track_list_t_destroy);
    self->sim_tracks_buffer = message_buffer_create((message_buffer_free_func_t) erlcm_track_list_t_destroy);
